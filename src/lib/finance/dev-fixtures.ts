...
//
// SEC-09/TS-07: this module used to import the repo's shared chainable
// test-helper fake (the one under the top-level tests directory), which
// put a test-only module on the production bundle graph behind nothing
// but a runtime env check. Below is a small, purpose-built fake instead —
// it only implements the handful of
// PostgREST methods the fetchers in queries.ts actually chain
// (select / eq / gte / contains / in / not / ilike / order / limit) and it
...
// --- inlined minimal chainable Supabase fake --------------------------------
...
export type BuilderState = FakeState;

type PgResult<T = unknown> = { data: T[]; error: null };
type TableResolver = (state: FakeState) => PgResult;
...
// other chained method (select / not / ilike / single / ...) is a no-op
// that still returns the same proxy, so a future call shape doesn't throw.
...
function makeBuilder(resolver: TableResolver): unknown {
  ...
  const handler: ProxyHandler<object> = {
    get(_t, prop) {
      ...
      if (prop === "then") {
        return (onFulfilled?: (v: PgResult) => unknown, onRejected?: (e: unknown) => unknown) =>
          Promise.resolve(resolver(state)).then(onFulfilled, onRejected);
      }
      return (...args: unknown[]) => {
        // Object.hasOwn (not `RECORD[prop]?.()`) — RECORD is a plain object
        // literal, so an unrecorded chained method name like "toString" or
        // "valueOf" resolves through Object.prototype to an inherited
        // function and gets invoked as a recorder with the wrong
        // arguments, and "__proto__" resolves to Object.prototype itself,
        // which optional-call does NOT guard against (it only short-
        // circuits null/undefined), so it throws instead of being the
        // no-op this fake promises for any other chained method.
        if (Object.hasOwn(RECORD, prop)) RECORD[prop]!(state, args);
...
function resolveTable(fixture: TableFixture | undefined, state: FakeState): PgResult {
  const result = typeof fixture === "function" ? fixture(state) : { data: fixture ?? [], error: null as null };
  const rows = result.data ?? [];
  return { data: state.limit != null ? rows.slice(0, state.limit) : rows, error: null };
}

interface FakeClientOptions {
  tables: Record<string, TableFixture>;
  rpc: Record<string, (args: unknown) => PgResult>;
}

function createFakeClient(options: FakeClientOptions): unknown {
  return {
    from: (table: string) => makeBuilder((state) => resolveTable(options.tables[table], state)),
    rpc: async (name: string, args?: unknown): Promise<PgResult> => {
      const fn = options.rpc[name];
      return fn ? fn(args) : { data: [], error: null };
    },
    auth: {
...
function eqVal(state: FakeState, col: string): unknown {
  return state.eq.find((e) => e.col === col)?.val;
}
function gteVal(state: FakeState, col: string): string | undefined {
  return state.gte.find((e) => e.col === col)?.val as string | undefined;
}
function containsVal(state: FakeState, col: string): string[] | undefined {
 ...
 * derived series (`day`, session bars, attention spikes). It all lives
 * inside `createFinanceFakeClient()` — not at module scope — so nothing is
 * built once at import time; each call gets a fresh `now` and the
 * fixtures' notion of "today" tracks the real clock.
 */
  ...
  return createFakeClient({
    tables: {
      articles: (state) => {
        ...
        return { data: ARTICLES.filter((a) => !t || a.article_tickers.some((x) => x.ticker === t)), error: null };
      },
      ticker_attention_daily: (state) => {
        const t = eqVal(state, "ticker");
        const since = gteVal(state, "day") ?? "0000";
        return { data: ATTENTION.filter((r) => (!t || r.ticker === t) && r.day >= since), error: null };
      },
      bist_companies: (state) => {
        const c = containsVal(state, "tickers");
        return { data: COMPANIES.filter((r) => !c || c.some((x) => r.tickers.includes(x))), error: null };
      },
      ...
      disclosure_coverage: (state) => {
        const t = eqVal(state, "ticker");
        return { data: COVERAGE.filter((r) => !t || r.ticker === t), error: null };
      },
      bist_quote_stats: (state) => {
        const list = inVals(state, "ticker") as string[] | undefined;
        return { data: QUOTE_STATS.filter((r) => !list || list.includes(r.ticker)), error: null };
      },
      bist_bars_5m: (state) => {
        const t = eqVal(state, "ticker");
        ...
        return { data: desc ? [...rows].reverse() : rows, error: null };
      },
    ...
    rpc: {
      ...
      econ_feed: (args) => {
        const { p_limit } = (args ?? {}) as { p_limit?: number };
        const data = [...ARTICLES]
